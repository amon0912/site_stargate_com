<div style="width: 100%;" id="vers-menu">
    <nav class="navbar navbar-expand-lg navbar-light sticky-top bg-light">
        <a class="navbar-brand pl-1" href="index.php">Stargate Communication</a>
        <button class="navbar-toggler d-lg-none" type="button" data-toggle="collapse" data-target="#collapsibleNavId" aria-controls="collapsibleNavId" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse pr-2" id="collapsibleNavId">
            <ul class="navbar-nav mr-auto mt-2 ml-2 mt-lg-0">
                <li class="nav-item active">
                    <a class="nav-link" href="#">Accueil<span class="sr-only">(current)</span></a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#">Nos Offres</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="dropdownId" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Nos Réalisations</a>
                    <div class="dropdown-menu" aria-labelledby="dropdownId">
                        <a class="dropdown-item" href="#">Action 1</a>
                        <a class="dropdown-item" href="#">Action 2</a>
                    </div>
                </li>
            </ul>
            <div class="my-0 ml-2 my-lg-0">
                <?php
                if (!empty($_SESSION['id'])) {
                    echo '<span style="color:orange; font-weight:bold;">'.$_SESSION["nom"].'<span>' .' <a href="?p=deconnexion" class=" btn btn-outline-primary">' .
                        ' <span>Se Déconnecter</span>
                    <i class="fas fa-user fa-fw "></i>
                </a>';
                } else {
                    echo '<a href="?p=connexion" class=" btn btn-outline-primary">
                    <span>Se connecter</span>
                    <i class="fas fa-user fa-fw "></i>
                </a>';
                }

                ?>

            </div>
        </div>
    </nav>
</div>